...
(function () {
  ...
  const prismThemeLink = document.getElementById(
    "prismTheme",
  ...
  const DEBOUNCE_MS: number =
    w.__DEBOUNCE_MS !== undefined ? w.__DEBOUNCE_MS : 150;
  ...
  function configureMarked(): void {
    ...
    try {
      ...
      if (
        typeof markedFootnote === "function" &&
  ...
  function highlightCodeBlocks(root: Element): void {
    ...
    blocks.forEach((block: Element) => {
      ...
      if (grammar) {
        (block as HTMLElement).innerHTML = Prism.highlight(
          block.textContent || "",
          grammar,
          lang,
        );
  ...
  function render(): void {
    ...
    const parser: any =
      typeof marked !== "undefined" && marked && (marked.parse || marked);
    let html = "";
  ...
  function applyTheme(pref: string): void {
    ...
    if (prismThemeLink) {
      prismThemeLink.setAttribute(
        "href",
        eff === "dark" ? PRISM_DARK : PRISM_LIGHT,
      );
    }
    if (themeBtn) {
      const label =
  ...
  function setThemePref(pref: string): void {
    try {
      if (w.localStorage) w.localStorage.setItem(THEME_KEY, pref);
    } catch (_) {}
    applyTheme(pref);
  }
  ...
  function applyLayout(mode: string): void {
    ...
    try {
      if (w.localStorage) w.localStorage.setItem(LAYOUT_KEY, mode);
    } catch (_) {}
  ...
  function applySplit(pct: number): void {
    ...
    try {
      if (w.localStorage) w.localStorage.setItem(SPLIT_KEY, String(pct));
    } catch (_) {}
  }
  ...
  function clearAllWorkspace(): void {
    ...
    try {
      ok = window.confirm(
        "Clear all workspace? This will permanently delete every file and folder.",
      );
    } catch (_) {}
    if (!ok) return;
    // Remove every per-doc key for current nodes.
    state.nodes.forEach((n) => {
      if (n.type === "file") safeRemove(DOC_PREFIX + n.id);
    });
    state = defaultState();
    expanded.clear();
    saveExpanded();
  ...
  if (clearAllBtn) clearAllBtn.addEventListener("click", clearAllWorkspace);
